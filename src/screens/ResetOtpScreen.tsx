...
export default function ResetOtpScreen({ setScreen }: { setScreen: SetScreen }) {
  ...
  return (
    <div style={{ backgroundColor: '#FFF8E7', minHeight: '100%', padding: '20px' }}>
      ...
      <div style={{ backgroundColor: '#fff', border: 'none', borderRadius: '16px', boxShadow: '5px 5px 0px #000', padding: '24px 20px' }}>
        <div style={{ display: 'flex', gap: '8px', justifyContent: 'center', marginBottom: '22px' }}>
          {otp.map((val, idx) => (
            <input key={`rotp_box_${idx}`} id={`rotp_${idx}`} type="tel" inputMode="numeric" maxLength={1} value={val} onChange={e => handleChange(idx, e.target.value)} onKeyDown={e => handleKey(idx, e)}
        ...
        <button onClick={() => setScreen('new_password')} style={{ width: '100%', fontFamily: 'Barlow Condensed, sans-serif', fontWeight: 700, fontSize: '17px', textTransform: 'uppercase', letterSpacing: '0.04em', backgroundColor: '#FFC50A', color: '#000', border: '2.5px solid #000', borderRadius: '12px', padding: '13px', cursor: 'pointer', boxShadow: '4px 4px 0px #000', marginBottom: '10px' }}>Verify →</button>
        <button style={{ width: '100%', fontFamily: 'Inter', fontWeight: 700, fontSize: '13px', background: 'none', border: 'none', opacity: 0.4, cursor: 'pointer', padding: '8px' }}>Resend OTP</button>
      </div>
    </div>
  )
}
