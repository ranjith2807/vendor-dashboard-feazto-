...
export default function ForgotPasswordScreen({ setScreen }: { setScreen: SetScreen }) {
  ...
  return (
    <div style={{ backgroundColor: '#FFF8E7', minHeight: '100%', padding: '20px' }}>
      ...
      <div style={{ backgroundColor: '#fff', border: 'none', borderRadius: '16px', boxShadow: '5px 5px 0px #000', padding: '20px' }}>
        {!sent ? (
          <>
            ...
            <div style={{ display: 'flex', gap: '8px', marginBottom: '18px' }}>
              <div style={{ backgroundColor: '#FFF8E7', border: 'none', borderRadius: '10px', padding: '0 12px', display: 'flex', alignItems: 'center', fontFamily: 'Inter', fontWeight: 700, fontSize: '15px' }}>+91</div>
              <input type="tel" placeholder="98765 43210" value={phone} onChange={e => setPhone(e.target.value.replace(/\D/g, '').slice(0, 10))} style={{ flex: 1, fontFamily: 'Inter', fontSize: '15px', backgroundColor: '#FFF8E7', border: '2.5px solid #000', borderRadius: '10px', padding: '12px 14px', outline: 'none' }} />
            </div>
            <button onClick={() => phone.length === 10 && setSent(true)} style={{ width: '100%', fontFamily: 'Barlow Condensed, sans-serif', fontWeight: 700, fontSize: '17px', textTransform: 'uppercase', letterSpacing: '0.04em', backgroundColor: phone.length === 10 ? '#FFC50A' : '#ddd', color: '#000', border: '2.5px solid #000', borderRadius: '12px', padding: '13px', cursor: 'pointer', boxShadow: phone.length === 10 ? '4px 4px 0px #000' : 'none' }}>Send OTP →</button>
          </>
        ) : (
          <div style={{ textAlign: 'center', padding: '10px 0' }}>
            ...
            <button onClick={() => setScreen('reset_otp')} style={{ width: '100%', fontFamily: 'Barlow Condensed, sans-serif', fontWeight: 700, fontSize: '17px', textTransform: 'uppercase', letterSpacing: '0.04em', backgroundColor: '#FFC50A', color: '#000', border: '2.5px solid #000', borderRadius: '12px', padding: '13px', cursor: 'pointer', boxShadow: '4px 4px 0px #000' }}>Enter OTP →</button>
          </div>
        )}
      </div>
    </div>
