  ...
  return (
    <ScrollView style={s.root} contentContainerStyle={s.body}>
      ...
      <View style={s.profileCard}>
        ...
        <TouchableOpacity
          style={[s.editBtn, activeBtn === 'edit' && activeStyle]}
          onPress={() => handleBtnPress('edit', () => setScreen('settings_profile'))}
        >
      ...
      <View style={s.quickRow}>
        {[
          ...
          <TouchableOpacity
            key={ql.id}
            style={[s.quickBtn, activeBtn === ql.id && activeStyle]}
            onPress={() => handleBtnPress(ql.id, () => setScreen(ql.screen))}
          >
      ...
      <View style={s.featureRow}>
        {[
          ...
          <TouchableOpacity
            key={f.id}
            style={[s.featureBtn, { backgroundColor: f.bg }, activeBtn === f.id && activeStyle]}
            onPress={() => handleBtnPress(f.id, () => setScreen(f.screen))}
          >
          ...
          <View style={s.sectionCard}>
            {section.items.map((item, idx) => {
              ...
              return (
                <TouchableOpacity
                  key={item.id}
                  style={[s.row, idx < section.items.length - 1 && s.rowBorder, isActive && activeStyle]}
                  onPress={() => {
                    if (!hasToggle && ITEM_NAV[item.id]) {
                      handleBtnPress(item.id, () => setScreen(ITEM_NAV[item.id]))
                    }
                  }}
      ...
      <TouchableOpacity
        style={[s.logoutBtn, activeBtn === 'logout' && activeStyle]}
        onPress={() => handleBtnPress('logout', () => setScreen('auth'))}
      >
...
const s = StyleSheet.create({
  ...
  editBtn: { borderRadius: 9, paddingHorizontal: 12, paddingVertical: 7, borderBottomWidth: 2.5, borderRightWidth: 2.5, borderColor: '#000000' },
  editBtnText: { fontFamily: F.interBold, fontSize: 12, color: C.black },
  quickRow: { flexDirection: 'row', gap: 8 },
  quickBtn: { flex: 1, backgroundColor: C.white, borderRadius: 12, borderBottomWidth: 2.5, borderRightWidth: 2.5, borderColor: '#000000', ...shadow(3, 3), paddingVertical: 12, alignItems: 'center', gap: 4 },
  quickLabel: { fontFamily: F.interBold, fontSize: 11, color: C.black },
  featureRow: { flexDirection: 'row', gap: 8 },
  featureBtn: { flex: 1,  borderRadius: 12, borderBottomWidth: 2.5, borderRightWidth: 2.5, borderColor: '#000000', ...shadow(3, 3), paddingVertical: 12, alignItems: 'center', gap: 3 },
