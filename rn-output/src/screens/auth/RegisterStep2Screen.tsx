...
export default function RegisterStep2Screen({ setScreen }: { setScreen: SetScreen }) {
  const [form, setForm] = useState({ kitchenName: '', cuisine: '', address: '', city: '' })
  const set = (k: string, v: string) => setForm(p => ({ ...p, [k]: v }))

  return (
    <ScrollView style={styles.container} contentContainerStyle={styles.content}>
      <View style={styles.header}>
        <TouchableOpacity onPress={() => setScreen('register_1')}><Text style={styles.back}>←</Text></TouchableOpacity>
        <View style={{ flex: 1 }}>
      ...
      <View style={styles.card}>
        {[
          { key: 'kitchenName', label: 'Kitchen Name', placeholder: "Priya's Kitchen" },
          { key: 'cuisine', label: 'Cuisine Type', placeholder: 'South Indian, Sweets…' },
          { key: 'address', label: 'Kitchen Address', placeholder: '14, Kamaraj St' },
